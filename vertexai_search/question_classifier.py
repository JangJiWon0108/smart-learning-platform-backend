...
# Gemini 모델에게 부여할 시스템 지시문(역할 및 규칙)
...
# 싱글톤으로 관리되는 Gemini API 클라이언트 인스턴스
...
def classify_question(question: str) -> dict[str, str]:
    ...
    s = Settings()
    # 1단계: Gemini API 모델 호출 (문제 텍스트 전송)
