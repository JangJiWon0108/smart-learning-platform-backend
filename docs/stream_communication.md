...
Frontend → POST /chat/stream
  ↓
FastAPI → routing workflow 스트리밍 실행 (execute_routing_stream)
  ↓
라우팅 결과(route) 결정 후 A2A route service `/stream`으로 요청 전달
  ↓
Route service → route workflow 스트리밍 실행 (execute_route_stream)
  ↓
Route service가 ADK Event를 프론트 SSE 계약으로 변환
  ↓
...
`chunk`는 실제 답변 텍스트다. 모든 노드에서 나오는 것이 아니라 `STREAM_NODES`에 포함된 노드에서만 나온다.

## 백엔드에서 보내는 위치

이벤트 전송은 “두 계층”에서 이루어진다.

- `api/app.py`의 `/chat/stream`: 라우팅 workflow 스트리밍을 먼저 내리고(emit_final=False), 이후에는 선택된 route의 A2A route service `/stream`을 호출해 SSE 텍스트를 그대로 전달한다.
- `a2a_remote_routes/services.py`의 `/stream`: route workflow 스트리밍을 실행하고, 프론트 SSE 계약(`state/chunk/curation/tracer/error/done`)으로 변환한다.

| type | 보내는 위치 | 조건 |
|---|---|---|
| `state` | `smart_learning_agent/streaming/frontend_events.py` (`iter_frontend_events`) | 새 노드 이름을 처음 만났을 때 |
| `chunk` | `smart_learning_agent/streaming/frontend_events.py` (`STREAM_NODES`) | 스트리밍 허용 노드에서 텍스트가 있을 때 |
| `stream_end` | `smart_learning_agent/streaming/frontend_events.py` | `chunk`를 보내던 노드가 끝났을 때 |
| `curation` | `smart_learning_agent/streaming/frontend_events.py` (`build_curation_payload`) | 추천 결과가 state에 있을 때 |
| `tracer` | `smart_learning_agent/streaming/frontend_events.py` (`build_tracer_payload`) | 시각화 결과가 state에 있을 때 |
| `error` | `a2a_remote_routes/services.py`(`/stream`) 또는 `api/app.py`(`/chat/stream`) | 준비/실행/전달 중 오류가 났을 때 |
| `done` | `a2a_remote_routes/services.py`(`/stream`) 및 `api/app.py`(`/chat/stream`) | 성공/실패와 관계없이 마지막 |
...
라우팅(workflow) + recommendation route(workflow)의 결합 흐름은 아래처럼 이해하면 된다.
...
intent_classification_agent
  ↓
intent_router
↓ route 결정(recommendation) 후 A2A route service `/stream` 호출
filter_agent
  ↓
vertex_search_agent
  ↓
...
`/chat/stream` 관점에서 보면 `intent_router`까지는 **메인 백엔드(라우팅 workflow)** 가 직접 스트리밍하고, 그 이후(`filter_agent`부터)는 **A2A route service(`/stream`)** 가 생성한 SSE를 그대로 전달하는 구조다. 프론트는 중간 경계가 있어도 동일한 이벤트 계약으로 연속 수신한다.
...
| 5 | `filter_agent` | `state` | 추천용 메타 필터를 만드는 중이라는 상태 문구 | LLM으로 `vertex_filter_output` 구성(MCP 없음) |
| 6 | `vertex_search_agent` | `state` | 추천 검색을 진행하는 중이라는 상태 문구 | MCP `search_exam_questions` 호출 |
...
여기서 모든 노드는 `state` 이벤트를 받을 수 있다. 하지만 `chunk`는 모든 노드에서 나오지 않는다. 현재 recommendation 흐름에서는 `STREAM_NODES`에 포함된 `curator_intro_agent`만 텍스트 조각을 `chunk`로 보낸다.
...
- 새 노드의 텍스트를 실시간으로 보여줘야 하면 백엔드 `STREAM_NODES`에 노드 이름을 추가한다.
